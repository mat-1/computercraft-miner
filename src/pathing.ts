...
export function getDirectionTo(position: vec3.Vec3, preferVisited?: boolean): vec3.DirectionUp {
	...
	if (preferVisited) {
		...
		if (position.north + 1 >= state.currentPosition.north) allowedDirections.push('north')
		if (position.north - 1 <= state.currentPosition.north) allowedDirections.push('south')
		if (position.east + 1 >= state.currentPosition.east) allowedDirections.push('east')
		if (position.east - 1 <= state.currentPosition.east) allowedDirections.push('west')
		if (position.up + 1 >= state.currentPosition.up) allowedDirections.push('up')
		if (position.up - 1 <= state.currentPosition.up) allowedDirections.push('down')
	}
	...
	for (const direction of allowedDirections) {
		...
		if (direction == state.currentDirection) directionScore += 2
